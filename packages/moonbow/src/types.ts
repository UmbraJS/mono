...
> extends MoonbowPipelineOptions {
  uniforms: (props: { target: import('./target').GPUCanvas; device: GPUDevice }) => Partial<U>
  storage: (props: { target: import('./target').GPUCanvas; device: GPUDevice }) => Partial<S>
  // Optional non-buffer resources: textures and samplers bound in @group(0)
  resources?: (props: { target: import('./target').GPUCanvas; device: GPUDevice }) => ResourceBinding[]
  bindGroups: import('./pipeline/core').BindGroups<U, S, B>
