...
const ButtonVariant = computed(() =>
  disabled ? buttonVariants.base : buttonVariants[variant],
);
...
<template>
  <component
    :is="ButtonVariant"
    tabindex="0"
    class="button buttonText buttonHover buttonActive buttonFocus"
    :class="[sizeClass, colorScheme]"
    :disabled="disabled"
...
@layer abstracts {
  ...
  .buttonGroup .button:not(:last-child, :first-child, :focus) {
    border-right-color: transparent;
    border-radius: 0px;
  }

  .buttonGroup .button:not(:focus):first-child {
    border-top-right-radius: 0px;
    border-bottom-right-radius: 0px;
    border-right-color: transparent;
  }

  .buttonGroup .button:not(:focus):last-child {
